...
do
    ...
    do
        ...
        if [[ $d -eq 0 ]]; then #twitter
            dataset=twitter; vertex=81344 edges=2420766
        elif [[ $d -eq 1 ]]; then #amazon
            dataset=amazon; vertex=262111 edges=1234878
        elif [[ $d -eq 2 ]]; then #LJ
            dataset=LVW; vertex=4847616; edges=68993773;
        elif [[ $d -eq 3 ]]; then #wiki
            dataset=wiki; edges=101311613; vertex=4206336;
        ...
        then
            ...
            config="sim-polygraph"
        elif [[ $a -eq 1 ]]; then
            # PR (priority ordering)
            param="OUTFILE=bins/page_$dataset.bin PR=1 FIFO=0 SPATIAL_STRIDE_FACTOR=1024 EPSILON=10"
            config="sim-blocked-async"
            #config="sim-polygraph"
        elif [[ $a -eq 2 ]]; then
            # BFS
        ...
        echo "PARAMS: "
        echo $param
        cmd="$config csr_file=\\\"$GRAPHSIM/sample_datasets/$dataset.tsv\\\" V=$vertex E=$edges $param"
        echo $cmd
        echo
        bin="0"
